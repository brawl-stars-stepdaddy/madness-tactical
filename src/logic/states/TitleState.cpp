...
TitleState::TitleState(StateStack &stack, State::Context context)
    ...
    m_brawl_start_stepdaddy_sprite.setPosition(
        get_context().window->getView().getSize() / 2.f -
        sf::Vector2f(
            0, m_brawl_start_stepdaddy_sprite.getLocalBounds().height / 2 + 20
        )
    );
    m_in_association_with_sprite.setPosition(
        get_context().window->getView().getSize() / 2.f +
        sf::Vector2f(
            0, m_in_association_with_sprite.getLocalBounds().height / 2 + 20
        )
    );
    m_presents_sprite.setPosition(
        get_context().window->getView().getSize() / 2.f
    );
}
...
void TitleState::draw() {
    ...
    if (m_current_time <= m_show_text_time * 2.f + sf::seconds(0.5)) {
        ...
        fade1.setSize(
            {m_brawl_start_stepdaddy_sprite.getLocalBounds().width,
             m_brawl_start_stepdaddy_sprite.getLocalBounds().height}
        );
        ...
        fade2.setSize(
            {m_in_association_with_sprite.getLocalBounds().width,
             m_in_association_with_sprite.getLocalBounds().height}
        );
        GuiUtil::center(fade2);
        fade2.setPosition(m_in_association_with_sprite.getPosition());

        fade1.setFillColor(sf::Color(
            0, 0, 0,
            sf::Uint8(
                255 - std::min(255.f, m_current_time * 255.f / m_show_text_time)
            )
        ));
        fade2.setFillColor(sf::Color(
            0, 0, 0,
            sf::Uint8(
                255 - std::min(
                          255.f, std::max(
                                     0.f, (m_current_time - m_show_text_time) *
                                              255.f / m_show_text_time
                                 )
                      )
            )
        ));
        ...
        sf::Time t1 =
            m_current_time - m_show_text_time * 2.f - sf::seconds(0.5);
        int n_logo = 1;
        ...
        if (n_logo != m_cur_logo) {
            m_cur_logo = n_logo;
            m_logo_texture.loadFromFile(
                "res/logos/" + std::to_string(n_logo) + ".png"
            );
            m_logo_sprite.setTexture(m_logo_texture, true);
            GuiUtil::resize(
                m_logo_sprite,
                sf::Vector2u(get_context().window->getView().getSize())
            );
        }
        ...
        fade.setFillColor(sf::Color(
            0, 0, 0, sf::Uint8((m_time_per_logo - t1) * 255.f / m_time_per_logo)
        ));
        ...
        fade.setFillColor(sf::Color(
            0, 0, 0,
            sf::Uint8(std::min(
                255.f, 255 - (m_time_per_logo - t1) * 255.f / m_time_per_logo
            ))
        ));
        get_context().window->draw(m_logo_sprite);
        get_context().window->draw(fade);
        ...
        sf::RectangleShape fade;
        fade.setSize(
            {m_presents_sprite.getLocalBounds().width,
             m_presents_sprite.getLocalBounds().height}
        );
        GuiUtil::center(fade);
