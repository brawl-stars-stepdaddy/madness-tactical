...
MoveLogicState::MoveLogicState(StateStack &stack, State::Context context, GameState &game_state) : State(stack, context), m_game_state(&game_state) {
    ...
    auto unit = std::static_pointer_cast<Unit>(m_game_state->get_team_manager()->get_active_team()->get_active_unit()->get_pointer());
    ...
    if (unit->get_weapon()->get_controller_type() == LAUNCH) {
        ...
        camera->set_follow_strategy(std::make_unique<SmoothFollowStrategy>(camera, unit));
    } else {
