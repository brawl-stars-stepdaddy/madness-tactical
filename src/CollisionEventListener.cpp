...
void CollisionEventListener::process(const EventData &event) {
    ...
    if (processed_collisions.find({collision_event.get_first_object(), collision_event.get_second_object()}) == processed_collisions.end()
            && processed_collisions.find({collision_event.get_second_object(), collision_event.get_first_object()}) == processed_collisions.end()) {
        ...
        processed_collisions.insert({collision_event.get_first_object(), collision_event.get_second_object()});
    }
}
