...
MoveTransitionLogicState::MoveTransitionLogicState(StateStack &stack, State::Context context, GameState &game_state)
    ...
    if (box_type) {
        box = std::make_shared<WeaponBox>(*m_game_state->get_world(), sf::FloatRect(position.x, position.y, 1.5, 1));
    }
    else {
        box = std::make_shared<HealingBox>(*m_game_state->get_world(), sf::FloatRect(position.x, position.y, 1.5, 1));
    }
    camera->set_follow_strategy(std::make_unique<SmoothFollowStrategy>(camera, box));
