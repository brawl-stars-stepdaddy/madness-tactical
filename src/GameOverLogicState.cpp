...
GameOverLogicState::GameOverLogicState(StateStack &stack, State::Context context, GameState &game_state) : State(stack, context), m_game_state(&game_state) {
    if (game_state.get_team_manager()->get_number_available_teams() == 0) {
        m_endgame_text.setString("Draw!");
    }
    else {
        m_endgame_text.setString("Win!");
        m_endgame_text.setFillColor(game_state.get_team_manager()->get_active_team()->get_team_color());
    }
    ...
    m_endgame_text.setPosition(get_context().window->getView().getSize().x / 2.f,
