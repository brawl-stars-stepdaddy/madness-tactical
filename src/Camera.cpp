...
SmoothFollowStrategy::SmoothFollowStrategy(Camera *camera, Entity *target, float min_speed, float convergence_factor)
    : m_camera(camera), m_target(target), m_min_speed(min_speed), m_convergence_factor(convergence_factor) {
}
...
void SmoothFollowStrategy::update(sf::Time delta_time) {
    ...
    sf::Vector2f delta_target2 = m_target->get_body().get_position() - m_camera->get_offset();
    float length2 = sqrt(pow(delta_target2.x, 2) + pow(delta_target2.y, 2));
    float delta_angle = m_target->getRotation() / (180.f / M_PI) - m_camera->get_angle();
    float delta_rotation = delta_angle / length * length2;
    m_camera->set_angle(m_target->getRotation() / (180.f / M_PI) - delta_rotation);
}
