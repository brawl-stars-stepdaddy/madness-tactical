...
void LandMine::draw_current(sf::RenderTarget &target, sf::RenderStates states) const {
    if (!m_is_hidden) {
...
void LandMine::launch() {
    ...
    m_world->get_layer(World::Layer::ENTITIES)->attach_child(std::make_unique<TimerProjectile>(
            *m_world, start_position, impulse, m_projectile_radius, m_explosion_radius, m_activation_time, m_explosion_time, true, true, TexturesID::LAND_MINE
    ));
}
